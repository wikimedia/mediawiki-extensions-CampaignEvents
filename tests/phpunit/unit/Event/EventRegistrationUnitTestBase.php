...
abstract class EventRegistrationUnitTestBase extends MediaWikiUnitTestCase {
	...
	abstract protected static function makeEventFromArguments( ...$args ): EventRegistration;

	public function testConstructAndGetters() {
		$data = static::getValidConstructorArgs();
		$registration = static::makeEventFromArguments( ...array_values( $data ) );
	...
	public function testInvalidTimestampFormat( array $constructorArgs, string $expectedWrongParam ) {
		...
		static::makeEventFromArguments( ...$constructorArgs );
	}

	public static function provideInvalidTimestampFormat(): Generator {
		yield 'Start timestamp' => [
			array_values( array_replace( static::getValidConstructorArgs(), [ 'start' => '1654000000' ] ) ),
			'$startLocalTimestamp'
		];
		yield 'End timestamp' => [
			array_values( array_replace( static::getValidConstructorArgs(), [ 'end' => '1654000000' ] ) ),
			'$endLocalTimestamp'
		];
	}
	...
	public static function provideEventsForUTCConversion(): Generator {
		$baseCtrArgs = static::getValidConstructorArgs();
		...
		yield 'Local in UTC' => [
			static::makeEventFromArguments( ...$replaceArgs( [
				'timezone' => new DateTimeZone( 'UTC' ),
				'start' => $summerStart,
				'end' => $summerEnd
			] ) ),
	...
	private static function getPastOngoingAndFutureEvents(): array {
		...
		$pastEvent = static::makeEventFromArguments(
			...array_values( array_replace(
				static::getValidConstructorArgs(),
				[ 'start' => $farPastTS, 'end' => $pastTS ]
			) )
		);

		$ongoingEvent = static::makeEventFromArguments(
			...array_values( array_replace(
				static::getValidConstructorArgs(),
		...
		$futureEvent = static::makeEventFromArguments(
			...array_values( array_replace(
				static::getValidConstructorArgs(),
